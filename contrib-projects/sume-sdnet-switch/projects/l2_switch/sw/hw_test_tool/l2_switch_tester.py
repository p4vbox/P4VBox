...
MAC_dst = "00:18:3e:02:0d:a1"
MAC_src = "00:18:3e:02:0d:a0"
vlan_id = 1
vlan_prio = 0
...
class TcpMonitorTester(cmd.Cmd):
    ...
    def _make_flow(self, flow_size):
        ...
        # make the SYN PKT
        pkt = Ether(dst=MAC_dst, src=MAC_src) / Dot1Q(vlan=vlan_id, prio=vlan_prio) / IP(src=srcIP, dst=dstIP) / TCP(sport=sport, dport=dport, flags='S')
        pkt = pad_pkt(pkt, MIN_PKT_SIZE)
        pkts.append(pkt)
        # make the data pkts
        size = flow_size
        while size >= PKT_SIZE:
            pkt = Ether(dst=MAC_dst, src=MAC_src) / Dot1Q(vlan=vlan_id, prio=vlan_prio) / IP(src=srcIP, dst=dstIP) / TCP(sport=sport, dport=dport, flags='A')
            pkt = pad_pkt(pkt, PKT_SIZE + HEADER_SIZE)
            pkts.append(pkt)
            size -= PKT_SIZE
        # make the FIN pkt
        size = max(MIN_PKT_SIZE - HEADER_SIZE, size)
        pkt = Ether(dst=MAC_dst, src=MAC_src) / Dot1Q(vlan=vlan_id, prio=vlan_prio) / IP(src=srcIP, dst=dstIP) / TCP(sport=sport, dport=dport, flags='F')
