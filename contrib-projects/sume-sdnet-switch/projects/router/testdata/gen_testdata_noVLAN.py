...
for i in range(6):
    ...
    # create input pkts
    pkt_app = Ether(src=src_MAC_app, dst=dst_MAC_app) / IP(ttl=64, chksum=0x7ce7, src=src_IP, dst=dst_IP) / TCP()
    ...
    pkt_exp = Ether(src=src_MAC_exp, dst=dst_MAC_exp) / IP(ttl=63, chksum=0x7ce7, src=src_IP, dst=dst_IP) / TCP()
