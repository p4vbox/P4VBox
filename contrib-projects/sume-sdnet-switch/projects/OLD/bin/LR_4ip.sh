...
# Fix files for run 4 parallel P4 IPs
cp -v $P4_PROJECT_DIR/testdata/gen_testdata_LR_4ip.py $P4_PROJECT_DIR/testdata/gen_testdata_LR.py
cp -v $NF_DESIGN_DIR/hw/hdl/nf_datapath_4ip.v $NF_DESIGN_DIR/hw/hdl/nf_datapath.v
cp -v $NF_DESIGN_DIR/hw/tcl/simple_sume_switch_4ip.tcl $NF_DESIGN_DIR/hw/tcl/simple_sume_switch.tcl
cp -v $NF_DESIGN_DIR/hw/tcl/simple_sume_switch_sim_4ip.tcl $NF_DESIGN_DIR/hw/tcl/simple_sume_switch_sim.tcl
...
# cd $P4_PROJECT_DIR && make config_writes 2>&1 | tee -a $P4_PROJECT_DIR/log/log_3-install_IPs_${P4_SWITCH}.txt
export P4_ID=2
export SDNET_OUT_DIR=nf_sume_sdnet_${P4_ID}_ip
echo ---------------- Compile P4: ${P4_SWITCH} - ${P4_ID} ------------------
cd $P4_PROJECT_DIR && make compile 2>&1 | tee -a $P4_PROJECT_DIR/log/log_2-build_P4_${P4_SWITCH}.txt
...
cd $SUME_SDNET/projects/bin/ && ./p4_build_ip.sh 2>&1 | tee -a $P4_PROJECT_DIR/log/log_2-build_P4_${P4_SWITCH}.txt
echo ------------- Install IP: ${P4_SWITCH} - ${P4_ID} ------------
cd $P4_PROJECT_DIR && make install_sdnet 2>&1 | tee -a $P4_PROJECT_DIR/log/log_3-install_IPs_${P4_SWITCH}.txt
# cd $P4_PROJECT_DIR && make config_writes 2>&1 | tee -a $P4_PROJECT_DIR/log/log_3-install_IPs_${P4_SWITCH}.txt
