...
for i in range(NUM_PKTS):
    ...
    # create create ping packets
    pkt_app = Ether(src=src_MAC_app, dst=dst_MAC_app) / Dot1Q(vlan=vlan_id, prio=vlan_prio) / IP(ttl=20, chksum=0x7ce7, src=src_IP, dst=dst_IP) / TCP()
    pkt_app = pkt_app/Raw('a'*(100-len(pkt_app)))
    pkt_app = pad_pkt(pkt_app, 256)
    ...
    pkt_exp = Ether(src=src_MAC_exp, dst=dst_MAC_exp) / Dot1Q(vlan=vlan_id, prio=vlan_prio) / IP(ttl=19, chksum=0x7ce7, src=src_IP, dst=dst_IP) / TCP()
    pkt_exp = pkt_exp/Raw('a'*(100-len(pkt_exp)))
    pkt_exp = pad_pkt(pkt_exp, 256)
