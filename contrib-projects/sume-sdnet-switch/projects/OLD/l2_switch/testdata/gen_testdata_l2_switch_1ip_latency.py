...
for i in numpy.arange(0, (NUM_PKTS/40.0), 0.1):
    ...
    pkt = Ether(src=MAC_addr[src_ind], dst=MAC_addr[dst_ind]) / Dot1Q(vlan=vlan_id, prio=vlan_prio) / IP(src=IP_addr[src_ind], dst=IP_addr[dst_ind], ttl=20) / TCP() / ('a'*(LEN_PKT-58))
