...
src_host_exp = 2        # packets sender host
...
# for time in numpy.arange(0, DEF_PKT_NUM, 1):
    ...
    dst_IP_exp = IP_addr_H[dst_host_map[src_host_exp]]

    if( vlan_id == vlan_id_map["nnp4"] ):
        src_MAC_app = MAC_addr_H[src_host]
        #### VLAN type
        ...
        src_MAC_exp = MAC_addr_S[dst_host_map[src_host]]
        dst_MAC = MAC_addr_H[dst_host_map[src_host]]
        dst_MAC_exp = MAC_addr_H[dst_host_map[src_host_exp]]
        # if ((dst_host_map[src_host] == 2) or (dst_host_map[src_host] == 3)):
        if (dst_host_map[src_host] == 3):
            #### Encap = 2 -> Desencapsulamento | Need modification in src/commands.txt
            src_MAC_app = MAC_addr_S[src_host-1]
            src_IP_app = IP_addr_H[src_host_exp]
            src_IP_exp = IP_addr_H[dst_host_map[src_host_exp+1]]
            pkt_app = Ether(src=src_MAC_app, dst=dst_MAC) / IP(src=src_IP_app, tos=200, dst=dst_IP, ttl=64, chksum=0x7ce7) / ProbeData(occup=1,neurondst=14, info=124, data=0000000) / ((DEF_PKT_SIZE - (HEADER_SIZE+17))*"A")
            pkt_exp = Ether(src=src_MAC_exp, dst=dst_MAC_exp) / IP(src=src_IP_exp, dst=dst_IP_exp, len=242, ttl=64, chksum=0x7ce7) / ((DEF_PKT_SIZE - HEADER_SIZE-17)*"A")
            extra_len = -LEN_PROBE
            pkt_app = pad_pkt(pkt_app, DEF_PKT_SIZE)
        # elif ((dst_host_map[src_host] == 0) or (dst_host_map[src_host] == 1)):
        elif (dst_host_map[src_host] == 1):
            #### Encap = 1 -> Encapsulamento | Need modification in src/commands.txt
            pkt_app = Ether(src=src_MAC_app, dst=dst_MAC) / IP(src=src_IP, dst=dst_IP, len=242, ttl=64, chksum=0x7ce7) / ((DEF_PKT_SIZE - HEADER_SIZE-17)*"A")
            # occup=0, neurondst=14, info=126, data=1193040
            pkt_exp = Ether(src=src_MAC_exp, dst=dst_MAC_exp) / IP(src=src_IP, tos=200, dst=dst_IP_exp, ttl=64, chksum=0x7ce7) / ProbeData(occup=1, neurondst=14, info=124, data=0000000) / ((DEF_PKT_SIZE - (HEADER_SIZE+17))*"A")
            extra_len = LEN_PROBE
        elif (dst_host_map[src_host] == 2): # 00:04:44 00:00:00:03:33
            #### Encap = 2 -> Desencapsulamento | Need modification in src/commands.txt
            pkt_app = Ether(src=src_MAC_app, dst=dst_MAC) / IP(src=src_IP, tos=200, dst=dst_IP, ttl=64, chksum=0x7ce7) / ProbeData(occup=1,neurondst=14, info=124, data=0000000) / ((DEF_PKT_SIZE - (HEADER_SIZE+17))*"A")
            pkt_exp = Ether(src=src_MAC_exp, dst=dst_MAC_exp) / IP(src=src_IP, dst=dst_IP_exp, len=242, ttl=64, chksum=0x7ce7) / ((DEF_PKT_SIZE - HEADER_SIZE-17)*"A")
            extra_len = -LEN_PROBE
            pkt_app = pad_pkt(pkt_app, DEF_PKT_SIZE)
            # elif ((dst_host_map[src_host] == 0) or (dst_host_map[src_host] == 1)):
        elif (dst_host_map[src_host] == 0):
            src_MAC_app = MAC_addr_S[src_host+1]
            src_IP_app = IP_addr_H[src_host_exp]
            src_IP_exp = IP_addr_H[dst_host_map[src_host_exp-1]]
            #### Encap = 1 -> Encapsulamento | Need modification in src/commands.txt
            pkt_app = Ether(src=src_MAC_app, dst=dst_MAC) / IP(src=src_IP_app, dst=dst_IP, len=242, ttl=64, chksum=0x7ce7) / ((DEF_PKT_SIZE - HEADER_SIZE-17)*"A")
            # occup=0, neurondst=14, info=126, data=1193040
            pkt_exp = Ether(src=src_MAC_exp, dst=dst_MAC_exp) / IP(src=src_IP_exp, tos=200, dst=dst_IP_exp, ttl=64, chksum=0x7ce7) / ProbeData(occup=1, neurondst=14, info=124, data=0000000) / ((DEF_PKT_SIZE - (HEADER_SIZE+17))*"A")
            extra_len = LEN_PROBE
    else:
        print("\nERROR: vlan_id not mapped!\n")
        exit(1)

    # if ((dst_host_map[src_host] == 0) or (dst_host_map[src_host] == 2)):
    # pkt_app = pad_pkt(pkt_app, DEF_PKT_SIZE)
    ...
    if ( src_host > (DEF_HOST_NUM-1) ):
        ...
        vlan_prio = 0
    src_host_exp += 1
    if ( src_host_exp > (DEF_HOST_NUM-1) ):
        src_host_exp = 0
