...
for time in range(DEF_PKT_NUM):
    ...
    if( vlan_id == vlan_id_map["nnp4"] ):
        ...
        #### Emcap = 2 -> Desencapsulamento | Need modification in src/commands.txt | Probe: data=1193040(hexa) -> 123450(decimal)
        pkt_app = Ether(src=src_MAC, dst=dst_MAC) / IP(src=src_IP, tos=200, dst=dst_IP, ttl=64, chksum=0x7ce7) / ProbeData(occup=1,neurondst=14, info=126, data=1193040) / ((DEF_PKT_SIZE - (HEADER_SIZE+17))*"A")
        src_MAC = MAC_addr_S[dst_host_map[src_host]]
        dst_MAC = MAC_addr_H[dst_host_map[src_host]]
        if (dst_MAC == "08:00:00:00:04:44"):
            #### Encap = 1 | Need modification in src/commands.txt
            # pkt_exp = Ether(src=src_MAC, dst=dst_MAC) / IP(src=src_IP, tos=200, dst=dst_IP, ttl=64, chksum=0x7ce7) / ProbeData(occup=1, neurondst=14, info=126, data=1193040) / ((DEF_PKT_SIZE - (HEADER_SIZE+17))*"A")
            # extra_len = LEN_PROBE
            #### Encap = 2 | Need modification in src/commands.txt
            pkt_exp = Ether(src=src_MAC, dst=dst_MAC) / IP(src=src_IP, dst=dst_IP, len=242, ttl=64, chksum=0x7ce7) / ((DEF_PKT_SIZE - HEADER_SIZE-17)*"A")
            extra_len = -LEN_PROBE
        else:
            #### Encap = 1 | Need modification in src/commands.txt
            # pkt_exp = Ether(src=src_MAC, dst=dst_MAC) / IP(src=src_IP, dst=dst_IP, ttl=64, chksum=0x7ce7) / ((DEF_PKT_SIZE - (HEADER_SIZE))*"A")
            #### Encap = 2 | Need modification in src/commands.txt
            pkt_exp = Ether(src=src_MAC, dst=dst_MAC) / IP(src=src_IP, tos=200, dst=dst_IP, ttl=64, chksum=0x7ce7) / ProbeData(occup=0, neurondst=14, info=126, data=1193040) / ((DEF_PKT_SIZE - (HEADER_SIZE+17))*"A")
            extra_len = 0
    else:
