...
class SimpleTester(cmd.Cmd):
	...
	dst_MAC = MAC_addr_H[dst_host_map[src_ind]]
        ...
        # make the data pkts
        vlan_prio = 0
        pkt = Ether(src=src_MAC, dst=dst_MAC) / IP(src=src_IP, dst=dst_IP, ttl=64, chksum=0x7ce7) / ((DEF_PKT_SIZE - HEADER_SIZE)*"A")
	...
	# sendpfast(pkts, pps=1000, loop=10000, iface=sender)
